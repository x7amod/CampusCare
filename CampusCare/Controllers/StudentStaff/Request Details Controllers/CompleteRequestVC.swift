...
class CompleteRequestVC: RequestDetailsBaseViewController {
    
    // IBOutlets specific to CompleteRequestPage
    ...
    private func populateRequestDetails(_ request: RequestModel) {
        ...
        if let completedDate = request.completedDate {
            requestCompleteDateLabel.text = dateFormatter.string(from: completedDate.dateValue())
        } else {
